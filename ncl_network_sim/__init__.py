...
from geo_functions import truncate_geom_funtion_maker
import pickle
...
def build_network(
    ...
    if speed_att is not None:
        ...
        while speed_field < len(sf.fields):
            if sf.fields[speed_field][0] == speed_att:
                # as the first field is not an actual attribute!
                speed_field -= 1
                break
                # if no speed field, will value will be greater than the number
                # of fields so will get an error
            speed_field += 1
        if speed_field == len(sf.fields):
            print "Could not find field titled '%s' containing the speed data.\
            Simulation will continue without speed being used." % (speed_att)
            speed_att = None

    if length_att is not None:
        ...
        while length_field < len(sf.fields):
            if sf.fields[length_field][0] == length_att:
                ...
                break
            length_field += 1
            # if no speed field, will value will be greater than the number of
            # fields so will get an error
        if length_field == len(sf.fields):
            print "Could not find field titled '%s' containing the length vales.\
            Simulation will calcualte the lengths accordingly." % (speed_att)
            length_att = None
    ...
    for shape in shapes:
        ...
        for stat in [stat1, stat2]:
            # takes every node and adds if not already added it to the list of
            # nodes
            if _truncate_geom(stat) not in out_nodes_geom_lookup_dict.keys():
                ...
                out_nodes_geom_lookup_dict[
                    node_class._truncated_geom] = node_class
                out_nodes.append(node_class)

        for i in range(len(shape.points) - 1):
            info = {}

            # add the edge straight to the network on the discovery that it has
            # not been added as yet
            speed = default_speed
            if speed_att is not None:
                # get the speed for the edge
                speed = sf.record(edge_count)[speed_field]

            if length_att is None:
                # calculate the length of the edge
                orig_e, orig_n = p1._truncated_geom
                dest_e, dest_n = p2._truncated_geom
                # I know this calculation can be done mch easier, but can't
                # remember how!
                temp1 = (orig_e - dest_e)
                temp2 = (orig_n - dest_n)
            ...
            p1, p2 = _truncate_geom(
                shape.points[i]), _truncate_geom(
                shape.points[
                    i + 1])
    ...
    for trunc, real in edge_lookup.iteritems():
        p1 = out_nodes_geom_lookup_dict.get(
            trunc[0],
            Node(
                real['geom'][0],
                trunc[0]))
        p2 = out_nodes_geom_lookup_dict.get(
            trunc[1],
            Node(
                real['geom'][1],
                trunc[1]))
        ...
        edge_geom_lookup_dict[
            (p1._truncated_geom, p2._truncated_geom)] = edge_class
...
def build_net_from_db(
    ...
    e_list = []
    n_list = []
    # reads the network and create a new one with all nodes and egdes for real
    # vis
    ...
    for nd in db_G.nodes_iter():
        ...
        char = 0
        while str(db_G.node[nd]['Wkt'])[char] != '(':
            char += 1
    ...
    for o, d in db_G.edges_iter():
        ...
        for coord in coordlist:
            ...
            while coord[char] != ' ':
                char += 1
            coordlist_tuples.append(
                [float(coord[:char]), float(coord[char + 1:])])

        coordlist = coordlist_tuples
        # to get bounds for bounding box, add to make a list of eastings and
        # northings
        for coord in coordlist:
            e, n = coord
            e_list.append(e)
        ...
        while j < len(coordlist) - 1:
            ...
            p1 = all_nodes_geom_lookup_dict.get(
                _truncate_geom(
                    coordlist[j]), Node(
                    coordlist[j], _truncate_geom(
                        coordlist[j])))
            p2 = all_nodes_geom_lookup_dict.get(
                _truncate_geom(
                    coordlist[
                        j + 1]),
                Node(
            ...
            if (p1._truncated_geom, p2._truncated_geom) not in orig_G.edges():
                ...
                # add the edge straight to the network if it has not been added
                # as yet
